...
export class Deployalizer {
    ...
    async getAllSLSTemplates(): Promise<any> {
        let scanResults = [];
        const params = {
            TableName: this.deploy.tableName,
            ProjectionExpression: "service_name, created_utc_ms, deploytime_utc_ms, cloudformationOutputs, template",
        ...
        this.deploy.deploytime_utc_ms = scanResults[scanResults.length - 1].deploytime_utc_ms;
        return scanResults;
    }
    ...
    async insertLatestSLSTemplate(): Promise<boolean> {
        ...
        const Item: DeployTableSchemaInterface = {
            ...
            cloudformationOutputs: this.deploy.cfOutputs
        };
